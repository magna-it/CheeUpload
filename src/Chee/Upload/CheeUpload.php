...
class CheeUpload
{
    ...
    */
    ...
    protected function pushError($key, $value)
    {
    ...
    public function move($dest)
    {
        ...
        if (! file_exists ($dest))
            if(! mkdir($dest,0777,true))
                $this->pushError('directory_path', 'directory path is invalid !');
        ...
        if ($this->file->move($this->savePath, $this->getName()))
            ...
            $this->pushError('move_file', 'file can not move');
    ...
    public function validation()
    {
        ...
        {
            $this->pushError('max_size', 'The uploaded file exceeds the post_max_size directive in php.ini');
            return false;
        }

        if (! $this->file->isValid())
            $this->pushError('invalid_file', 'this is not a file !');
    ...
    private function addMaxHeightRule()
    {
        $this->app['validator']->extend('maxHeight', function($attribute, $value, $parameters)
        {
            ...
            if (! $imageHeight)
            {
                $this->pushError('image_dimension', 'image dimension not found!');
                return false;
            }
